...
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ 
    ...
    def sameSpecies(self, g):
        compatibility = 0.0
        excessAndDisjoint = getExcessDisjoint(g, self.rep) # get the number of excess and disjoint genes between this player and the current species rep
        averageWeightDiff = averageWeightDiff(g, self.rep) # get the average weight difference between matching genes
        
        largeGenomeNormalizer = len(g.genes) - 20
        if largeGenomeNormalizer < 1:
            largeGenomeNormalizer =1

        compatibility =  (excessCoeff* excessAndDisjoint / largeGenomeNormalizer) + (weightDiffCoeff* averageWeightDiff) #compatablilty formula
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def sortSpecies(self):
        ...
        for i in range(len(self.players)):
            ...
            temp.add(self.players[maxIndex])
...
#------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def fitnessSharing(self):
        for i in range(len(self.players)):
            self.players[i].fitness /= len(players)